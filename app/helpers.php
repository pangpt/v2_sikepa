 ...
 * ------------------------------------------------------------------------
 */
if (!function_exists('UserLogAccess')) {
  /**
   * Log user access to a page.
   *
   * @return void
   */
  function UserLogAccess($text = '')
    {
        $auth_text = '';
        $user_ip = request()->ip(); // Mendapatkan IP pengguna saat ini

        // Check if the user is authenticated
        if (auth()->check()) {
            // Get the authenticated user
            $user = auth()->user();
            // Create a string with the user's name and ID
            $auth_text = "User:{$user->employee->nama} (ID:{$user->id}) | IP: {$user_ip}";
        }

        // Log the access with the custom text and user information
